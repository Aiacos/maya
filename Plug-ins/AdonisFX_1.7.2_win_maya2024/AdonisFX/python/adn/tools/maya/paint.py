...
def get_initial_shading_group_uuid():
    """Get the UUID of the initial shading group. This is used to check if the current shading group
    is the initial one or not.

    Returns:
        str: UUID of the initial shading group if it exists, None otherwise.
    """
    init_shading_group_uuid = cmds.ls("initialShadingGroup", uuid=True)
    if not init_shading_group_uuid:
        return None
    return init_shading_group_uuid[0]


def get_shading_group_uuid_members(mesh_shape):
    """
    Get the shading group UUID and its members for the given mesh shape.
    An example of the dictionary is:
    {
        "12345678-1234-1234-1234-123456789012": ["pSphereShape1.f[3]", "pSphereShape1.f[4]"],
        "23456789-2345-2345-2345-234567890123": ["pSphereShape1.f[5]", "pSphereShape1.f[6]"]
    }
    Where the key is the shading group UUID and the value is a list of
    members that belong to that shading group.
    This is used to restore the original shading group for restoring
    the original material after painting.

    Args:
        mesh_shape (str): Mesh shape name to get the shading group UUID and its members from.

    Returns:
        dict: Dictionary with shading group UUIDs as keys and lists of members as values.
    """
    if not mesh_shape:
        return {}
    # Ensure the mesh shape is in long format, the input to this method should be either the short
    # name provided by cmds.ls or the long name that is automatically generated by Maya.
    # If the input is already in long format, it will not change.
    mesh_shape = cmds.ls(mesh_shape, long=True, dag=True, objectsOnly=True, shapes=True)
    if not mesh_shape:
        return {}

    # We do not support multiple mesh shapes in this method
    if len(mesh_shape) > 1:
        return {}
    mesh_shape = mesh_shape[0]

    # Get the transform node of the mesh shape for shading group handling
    # This is needed since the shading groups can be stored with the transform node
    # name or the shape node name.
    mesh_transform = cmds.listRelatives(mesh_shape, parent=True, fullPath=True) or []
    if len(mesh_transform) > 0:
        mesh_transform = mesh_transform[0]

    # Get all unique shading groups connected to the shape
    all_sgs = cmds.listConnections(mesh_shape, type="shadingEngine") or []
    all_sgs = list(set(all_sgs))

    if not all_sgs:
        # No shading groups found, return empty dictionary
        return {}

    # Iterate through all shading groups and find the ones that have members
    # and store it in a dictionary
    sg_uuid_members = {}
    for shading_group in all_sgs:
        # Get the members of the shading group
        members = cmds.sets(shading_group, query=True) or []
        if not members:
            continue
        members_on_this_shape = []
        for m in members:
            root = m.split('.')[0]
            root = cmds.ls(root, long=True)[0]
            if root == mesh_shape or root == mesh_transform:
                # m might be "pSphere1" or "pSphereShape1" or "pSphereShape1.f[3]" or "pSphere1.f[3]"
                members_on_this_shape.append(m)
            elif m.startswith("{0}.f[".format(mesh_shape)):
                # check if “m” is a face of mesh_shape, e.g. "pSphereShape1.f[5]" or "pSphere1.f[2:10]"
                members_on_this_shape.append(m)
        # If there are no members on this shape, skip to the next shading group
        if not members_on_this_shape:
            continue
        # Get current shading group UUID to avoid naming discrepancies
        ...
        if not shading_group_uuid:
            continue
        # Store the shading group UUID and its members in the backup dictionary
        sg_uuid_members[shading_group_uuid] = members_on_this_shape
    return sg_uuid_members


def set_shading_group_uuid_members(mesh_shape, sg_uuid_to_restore):
    """
    Restore the shading groups to the given mesh shape. This is used to restore the original
    shading group after painting.

    Args:
        mesh_shape (str): Mesh shape name to restore the shading groups to.
        sg_uuid_to_restore (dict): Dictionary with shading group UUIDs as keys and lists of members as values.
        An example of the dictionary is:
        {
            "12345678-1234-1234-1234-123456789012": ["pSphereShape1.f[3]", "pSphereShape1.f[4]"],
            "23456789-2345-2345-2345-234567890123": ["pSphereShape1.f[5]", "pSphereShape1.f[6]"]
    ...
    Returns:
        bool: True if all shading groups were restored successfully. False otherwise.
    """
    if not mesh_shape:
        return False
    # Ensure the mesh shape is in long format, the input to this method should be either the short
    ...
    counter = 0
    # Restore the shading groups to the painted object
    for sg_uuid, member_list in sg_uuid_to_restore.items():
        valid = False
        ...
        if not valid:
            continue
        original_shading_group = cmds.ls(sg_uuid, long=True)
        if not original_shading_group:
            continue
        # Restore the initial shading group before painting started
        cmds.sets(member_list, edit=True, forceElement=original_shading_group[0], noWarnings=True)
        counter += 1
    # It was not possible to restore all the shading groups
    if counter != len(sg_uuid_to_restore):
        return False
    return True
...
def assign_default_material_multi(mesh_node):
    """Switch to a default material in case the mesh element sent was not using the default
    "initialShadingGroup". The original shading group sets from the object will be returned.
    This ensures that the user can use the paint tool with DirectX shaders and other
    ...
    Returns:
        dict: Node UUIDs of the original shading groups nodes if possible,
            Empty dict otherwise.
    """
    if not mesh_node:
        return {}

    # Get for each shading group the UUID and its members in a dictionary
    sg_uuid_backup = get_shading_group_uuid_members(mesh_node)

    # Avoid returning if there is no shading group to restore
    if not sg_uuid_backup:
        return {}

    # Get the initial shading group UUID
    init_shading_group_uuid = get_initial_shading_group_uuid()

    # Avoid setting the initial shading group if we already have it on the whole mesh
    if len(sg_uuid_backup) == 1 and init_shading_group_uuid in sg_uuid_backup:
        return sg_uuid_backup
    
    # Combine all members from the shading groups into a single list to be set to the initial shading group
    members_list_combined = [m for member_list in sg_uuid_backup.values() for m in member_list]

    # noWarning flag added since elements of the rig might not totally accept
    # the set change but allow it
    cmds.sets(members_list_combined, edit=True, forceElement="initialShadingGroup", noWarnings=True)
    return sg_uuid_backup


def restore_material_multi(mesh_shape, sg_uuid_to_restore):
    """
    Attempts to restore the given shading groups to the given mesh. This assignment is applied
    only if the current shading group of that mesh is the default one.

    Args:
        mesh_shape (str): Mesh node to change the material from.
        sg_uuid_to_restore (dict): Dictionary with shading group UUIDs as keys and lists of members as values.
        An example of the dictionary is:
        {
            "12345678-1234-1234-1234-123456789012": ["pSphereShape1.f[3]", "pSphereShape1.f[4]"],
    ...
    Returns:
        bool: False if there was an error restoring the material. True otherwise.
    """
    if not sg_uuid_to_restore or not mesh_shape:
        return False

    # Get for each shading group the UUID and its members in a dictionary
    sg_uuid_backup = get_shading_group_uuid_members(mesh_shape)
    ...
    # Check if the current shading group is different from default one
    if len(sg_uuid_backup) != 1 or init_shading_group_uuid not in sg_uuid_backup:
        # Only one shading group and it is the initial one
        return False
    return set_shading_group_uuid_members(mesh_shape, sg_uuid_to_restore)
...
def display_paint(mesh_node, enable, current_shading_group_uuid, poly_option):
    ...
    Args:
        ...
        current_shading_group_uuid (str or dict): If the input is a string it is assumed to be one single
            shading group UUID. If the input is a dictionary it is assumed to be a dictionary with
            shading group UUIDs as keys and lists of members as values.
    ...
    Returns:
        str or dict: node UUID of the original shading group if the material has been
            switched, or a dictionary with the shading group UUIDs as keys and lists of members as values
            of the original shading nodes if the material has been
    ...
    # Check if the input is a dictionary or a string to use the UUID
    # directly for restoring the material or use the dictionary
    is_string = isinstance(current_shading_group_uuid, str)
    ...
    if enable:
        if not current_shading_group_uuid:
            # Switch material to default one
            if is_string:
                shading_group_uuid = assign_default_material(mesh_node)
            else:
                shading_group_uuid = assign_default_material_multi(mesh_node)
        else:
            # Material already switched
            shading_group_uuid = current_shading_group_uuid
        # Get the colorMaterialChannel of the painted mesh
        poly_options_result = cmds.polyOptions(mesh_node,
                                               colorMaterialChannel=True,
                                               query=True)
        previous_poly_option = poly_options_result[-1] if poly_options_result else "none"
        ...
        if is_string:
            ...
            success = restore_material_multi(mesh_node, current_shading_group_uuid)
