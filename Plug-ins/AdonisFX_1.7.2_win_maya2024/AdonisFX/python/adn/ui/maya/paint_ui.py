...
from adn.utils.maya.checkers import is_deformer_on_lowest_init_time
from adn.utils.maya.constants import PaintToolAttrDisableRule
from adn.commands.maya.scene import get_deformer_times
from adn.utils.maya.constants import PaintToolAttrDisableRule
...
class PaintToolUI(BaseWidget):
    ...
    def __init__(self, deformer, node_type, paintable_attributes, multi_infl_attr, 
        ...
        self.paint_attr_list_only_on_low_init = dcc_tool.get_on_low_init_only_paintable_attributes()
        ...
        # Define the rules to follow for disabling attributes in the UI
        self.disable_rules = [PaintToolAttrDisableRule.kAttrOnLowestInitTimeOnly]
        ...
        self._disable_attr() 
        self._switch_disabled_attr(attr_changed=False)
        ...
        if (self.attr_combo.currentText() not in self.multi_infl_attr or
            ...
            self._paint_ctx_setup_procedure()
            # Select multi-influence object if we are in a multi-influence attribute.
        ...
        self.dcc_tool.select_debug_locator()
    ...
    def _build_ui(self):
        ...
        self._reset_action = QAction("&Reset tool", self, triggered=self._reset)
    ...
    def _connect_signals(self):
        ...
        # Shortcuts
        self.flood_sc = QShortcut(QtGui.QKeySequence("Alt+F"), self,
                                  context=QtCore.Qt.ApplicationShortcut)
        self.flood_sc.activated.connect(self._flood_weights)
        self.color_feedback_sc = QShortcut(QtGui.QKeySequence("Alt+C"), self,
                                           context=QtCore.Qt.ApplicationShortcut)
        self.color_feedback_sc.activated.connect(self._color_feedback_sc_changed)
        self.reflection_sc = QShortcut(QtGui.QKeySequence("Alt+R"), self,
                                       context=QtCore.Qt.ApplicationShortcut)
    ...
    def _projection_changed(self, value):
        ...
        """        
        if cmds.artUserPaintCtx(self.TOOL_PAINT, exists=True):
            cmds.artUserPaintCtx(self.TOOL_PAINT, edit=True, projective=QtCore.Qt.CheckState(value) == QtCore.Qt.Checked)
    ...
    def _reflection_origin_changed(self, value):
        ...
        """        
        if cmds.artUserPaintCtx(self.TOOL_PAINT, exists=True):
            cmds.artUserPaintCtx(self.TOOL_PAINT, edit=True, reflectionaboutorigin=QtCore.Qt.CheckState(value) == QtCore.Qt.Checked)
    ...
    def _paint(self):
        ...
        report_error(result, "launching the AdonisFX Paint Tool")
        if not result:
            return
        ...
        # Get a list of the attributes that can only be paintable during lowest initialization time.
        paint_attr_only_on_low_init = self.dcc_tool.get_on_low_init_only_paintable_attributes()
        self._refresh_cache(deformer, node_type, paint_attr,
                            paint_attr_only_on_low_init,
    ...
    def _refresh_cache(self, deformer, node_type, paint_attr,
                       paint_attr_only_on_low_init, 
                       multi_infl_attr, mesh_node):
        ...
        Args:
            ...
            paint_attr_only_on_low_init (list): List of paintable attributes only 
                                                paintable on the lowest init time.
        ...
        self.paint_attr_list_only_on_low_init = paint_attr_only_on_low_init
        self.multi_infl_attr = multi_infl_attr
        self.attr_combo.blockSignals(True)
    ...
    def _paint_ctx_setup_procedure(self, toolContext=None):
        ...
        if deformer != self.deformer:
            # Get a list of the attributes that can only be paintable during initialization time.
            paint_attr_only_on_low_init = self.dcc_tool.get_on_low_init_only_paintable_attributes()
            self._refresh_cache(deformer, node_type, paint_attr,
                                paint_attr_only_on_low_init,
                                multi_infl_attr, mesh_node)
        ...
        # Make the deformer enter in display mode
        self.dcc_tool.toggle_display_weight(deformer, True)
    ...
    def _paint_ctx_cleanup_procedure(self, toolContext=None):
        ...
        """
        # Make the deformer exit from display mode
        self.dcc_tool.toggle_display_weight(self.deformer, False)
        ...
        current_selection = cmds.ls(selection=True, long=True)
        _, _, mesh_node, _ = self.dcc_tool.collect_paint_tool_nodes(current_selection)
        if mesh_node and cmds.objExists(self.mesh) and self.mesh_node == mesh_node:
    ...
    def _show_paint_weights(self):
        ...
        if not self.mesh_node or self.colors_active:
            return
        self._shading_group_uuid, self._poly_option, result = self.dcc_tool.display_paint(self.mesh_node, True,
                                                                                          self._shading_group_uuid,
                                                                                          self._poly_option)
        report_error(result, "displaying painted weights")
    ...
    def _hide_paint_weights(self):
        ...
        if not self.mesh_node or not self.colors_active:
            return
        _, _, result = self.dcc_tool.display_paint(self.mesh_node, False, self._shading_group_uuid, self._poly_option)
        report_error(result, "hiding painted weights")
        self._shading_group_uuid = {}
        self._poly_option = "none"
        self.colors_active = False
    ...
    def _disable_attr(self):
        """Disable certain attributes in the attribute combobox given certain disable rules.
        Providing disable rules allows for scalability in case new attribute rules have to be
        defined. Eg. disable attributes whenever the deformer is disabled for example.
        """
        num_attr = len(self.paintable_attrib_list)
        if self.attr_combo.count() != num_attr:
            return
        if not self.disable_rules:
            return
        model = self.attr_combo.model()

        # Enable all attributes and remove tooltips
        for i in range(num_attr):
            model.item(i).setEnabled(True)
            self.attr_combo.setItemData(i, "", QtCore.Qt.ToolTipRole)
            self.attr_combo.setItemIcon(i, QtGui.QIcon())

        # Lowest init time is either the lowest between the preroll and start time
        is_on_lowest_init_time = is_deformer_on_lowest_init_time(self.deformer)

        # Iterate over the paintable attributes
        for i, attr in enumerate(self.paintable_attrib_list):
            warning_msg_list = list()
            # Iterate over the disable rules
            for rule in self.disable_rules:
                # On the lowest init time only disable rule
                if rule == PaintToolAttrDisableRule.kAttrOnLowestInitTimeOnly:
                    if not is_on_lowest_init_time and attr in self.paint_attr_list_only_on_low_init:
                        preroll, start, current = get_deformer_times(self.deformer)
                        init_time = min(preroll, start)
                        # Disable
                        model.item(i).setEnabled(False)
                        # Warning
                        warning_msg = ("Warning: '{0}' attribute is not paintable at the current ({1}) time.\n"
                                       "Please go to ({2}) time for the attribute to become paintable again."
                                       "".format(attr, current, init_time))
                        warning_msg_list.append(warning_msg)
            # Set the warning only if a disable rule has been triggered
            if warning_msg_list:
                out_warning_msg = "\n".join(warning_msg_list)
                self.attr_combo.setItemData(i, out_warning_msg, QtCore.Qt.ToolTipRole)
                app_style = QtWidgets.QApplication.instance().style()
                warning_icon = app_style.standardIcon(QtWidgets.QStyle.SP_MessageBoxWarning)
                self.attr_combo.setItemIcon(i, warning_icon)
                        
    
    def _switch_disabled_attr(self, attr_changed=False):
        """Disabling certain attributes in the attribute combobox is supported. If the current
        selected attribute in the attribute combobox was disabled switch to the first available attribute
        and force the update of the context.
        
        Args:
            attr_changed (bool, optional): Trigger the attribute changed logic or not
                when the attribute has to change. Defaults to False.
                Eg. When switching to a different attribute in the UI the attr_changed method will be 
                triggered automatically to gather the value from the UI and set the corresponding
                value to the deformer. As we are switching the disabled attribute before handling 
                the attribute changed event we use attr_changed == False as we assume that the uber
                method is the one in charge of calling that method afterwards. However, the situation
                in which attr_changed == True is when we call this method from the onEnter event of the
                UI as we want to block the signals and then refresh the value in the deformer only
                if strictly necessary (when the attribute needs to switch).
        """
        num_attr = len(self.paintable_attrib_list)
        if self.attr_combo.count() != num_attr:
            return
        ...
        # Get the attribute ids that are enabled
        enabled_attr_ids = [i for i in range(num_attr) if model.item(i).isEnabled()]
        if not enabled_attr_ids:
            return

        # Get the first enabled attribute id
        first_enabled_attr_id = enabled_attr_ids[0]
        current_attr = self.attr_combo.currentText()

        # Iterate over the paintable attributes
        for i, attr in enumerate(self.paintable_attrib_list):
            enabled = model.item(i).isEnabled()
            if attr == current_attr and not enabled:
                self.attr_combo.blockSignals(True)
                self.attr_combo.setCurrentIndex(first_enabled_attr_id)
                self.attr_combo.blockSignals(False)
                if attr_changed:
                    self._attr_changed()
                return
    ...
    def enterEvent(self, event):
        ...
        self._disable_attr()
        self._switch_disabled_attr(attr_changed=True)
        ...
        # Add the debug locator to the selection
        self.dcc_tool.select_debug_locator()
        self.entering_ui = False
