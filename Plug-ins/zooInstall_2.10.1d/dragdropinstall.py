...
class InstallDialog(QtWidgets.QDialog):
    ...
    def _createWidgets(self):
        ...
        self.modulePathCombo = ModCombo(self.core, parent=self, initText=modPath)
    ...
    def createLayouts(self):
        ...
        customizeLayout.addWidget(QtWidgets.QLabel(
            "    <i>All the core Zoo Tools scripts are stored in the scripts path under the folder 'zootoolspro'</i>", parent=self),
            r + 1, 1)
        ...
        customizeLayout.addWidget(QtWidgets.QLabel(
            "    <i>The modules path with the file 'zootoolspro.mod'. Tells maya where the scripts are stored.</i>", parent=self),
            r + 1, 1)
        ...
        customizeLayout.addWidget(QtWidgets.QLabel(
            "    <i>The 'zoo_preferences' folder that stores personal settings, presets and assets.</i>", parent=self), r + 1, 1,)
    ...
    def browseScriptsPath(self):
        ...
        directoryPath = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Set Scripts Directory", self.scriptsPathEdit.text()
        )
    ...
    def browseModulesPath(self):
        ...
        """
        directoryPath = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Set Modules Directory", self.modulePathCombo.currentPath()
        )
        self.sender().setDown(False)  # unstuck the button
        if not directoryPath:
            return
    ...
    def browsePrefsPath(self):
        ...
        directoryPath = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Set Preferences Directory", path
        )
        self.sender().setDown(False)  # unstuck the button
        if not directoryPath:
            return
    ...
    def installUpgrade(self):
        ...
        if self.modulePathCombo.currentPath() not in self.core.modDirs():
            ...
            res = ui.MessageBoxQt.showQuestion(
                self,
                "Non-Maya Module Path",
                message=message,
                buttonA="Yes",
        ...
        if self.core.isNewerVersion() == "NO":
            ...
            installed = self.core.installedVersion()
            version = self.core.versionPretty()
    ...
    def connections(self):
        ...
        :return:
        """
        self.scriptsBrowseBtn.clicked.connect(self.browseScriptsPath)
        self.moduleBrowseBtn.clicked.connect(self.browseModulesPath)
        self.prefBrowseBtn.clicked.connect(self.browsePrefsPath)
        self.helpButton.clicked.connect(self.openHelp)
        self.installButton.clicked.connect(self.installUpgrade)
        self.expressButton.clicked.connect(self.expressClicked)
        self.customizeButton.clicked.connect(self.customizeClicked)


class GettingStartedDialog(ui.ImageDialog):
    def __init__(self):
        super(GettingStartedDialog, self).__init__(
            parent=utils.mayaMainWindow(),
...
def checkFileLength(path):
    ...
    if len(path) + zooPathLimit > OS_PATH_LIMIT:
        text = (
            "The install location is too long as {} is limited to {} characters and zootools has long paths. "
            "\n\nPlease move this directory to a shorter path length such as:"
            "\n\nC:\\users\\yourname\\downloads\\zootoolspro_version".format(
                platform.system(), OS_PATH_LIMIT
            )
        )
        ui.errorPopup(text)
        return False
