...
from zoo.libs.maya.api import attrtypes, nodes as apinodes
from zoo.libs.maya.meta import base
from zoo.libs.maya.cmds.objutils import curves as cmdsCurves
...
from zoo.libs.hive.base.definition import defutils, driverutils, spaceswitch
...
class Component(object):
    ...
    BuildSystem Methods:
        ...
        * :func:`Component.deformationJointIds`
    ...
    def namingConfiguration(self):
        ...
        globalDefault = self.configuration.namePresetRegistry().namingConfigs[
            constants.ZOOTOOLS_GLOBAL_CONFIG
        ]
        # Because naming configuration hierarchy is configured only ever once per zoo session and
    ...
    def setSide(self, side):
        ...
        self._updateSpaceSwitchComponentDependencies(name, oldSide, self.name(), self.side())
    ...
    def updateJointColours(self, deformJoints, nonDeformJoints, colour):
        ...
        for jnt in allJoints:
            ...
            for child in jnt.iterChildren(recursive=False, nodeTypes=(zapi.kJoint,)):
                if not child.overrideEnabled.value() and child not in allJoints:
