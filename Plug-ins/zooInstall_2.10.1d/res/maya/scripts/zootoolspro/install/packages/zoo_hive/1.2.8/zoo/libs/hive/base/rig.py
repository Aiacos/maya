...
class Rig(object):
    ...
    def __init__(self, config=None, meta=None):
        ...
        self._zooVersion = zooapi.currentConfig().buildVersion()
        self._hiveVersion = ""

    def versionInfo(self):
        """Returns the build version info for the rig. Include, hive version, maya version, zoo version.
...
def _setupDrivers(components):
    """Internal method which loops over the provided components"""
    for comp in components:
        with mayageneral.namespaceContext(comp.namespace()):
            container = comp.container()  # type: zapi.ContainerAsset
            if container is not None:
