...
def _runZooMarkingMenu(selectedNode):

    # get the current menu path so zoo can clear the existing default items
...
def resetMarkingMenuTrigger():
    cmds.callbacks(clearCallbacks=True, owner='ZooToolsPro')
...
def setupMarkingMenuTrigger():
    ...
    """
    cmds.callbacks(addCallback=_runZooMarkingMenu, hook='addRMBBakingMenuItems', owner='ZooToolsPro')
