...
class ImageDialog(QtWidgets.QDialog):
    ...
    def __init__(
        ...
        buttonA="OK",
        buttonB="",
        buttonIconA=const.Images.CHECKMARK,
        buttonIconB=None,
        width=None,
        modal=False,
        clickable=False,
    ...
    def imageLayout(self, parent):
        ...
        if width:
            pixmap = pixmap.scaledToWidth(
                width - utils.dpiScale(10), QtCore.Qt.SmoothTransformation
            )
...
def errorPopup(errorMessage, parent=None):
    ...
    QtWidgets.QMessageBox.warning(
        parent or utils.mayaMainWindow(), "Error!", errorMessage, flags
    )
...
def saveCurrentScene(
    ...
    """
    buttonPressed = MessageBoxQt.showQuestion(
        title=title,
        parent=parent,
        message=message,
        buttonA=buttonA,
...
def makeDirectory(directory, parent=None):
    ...
    try:
        ...
        message = "The directory could not be created: {} \n\n{}".format(
            directory, const.DID_NOT_INSTALL
        )
        om2.MGlobal.displayError(message)
        errorPopup(message, parent=parent)
        return False
...
def moveRootLocation(rootPath, destinationPath, parent=None):
    ...
    try:
        ...
        logger.error(
            "Failed to remove the preference root: {}".format(rootPath), exc_info=True
        )
        raise
    return True
