...
class InstallerCore(object):
    ...
    def loadZooPlugin(self, rootDirectory):
        ...
        :return:
        """
        mayaVersion = utils.mayaVersion()
        if mayaVersion >= 2022:
            securityDialog = ImageDialog(parent=self.installDialog,
                                         imageFile=Images.ALLOW_PLUGIN,
                                         text="Maya {} requires users to allow new plugins.\n"
                                              "Please allow permissions for Zoo Tools Pro.".format(mayaVersion),
                                         title="Allow Plugin",
    ...
    def version(self):
        ...
        return version.LooseVersion(zooJson['version'])
        ...
        if os.path.isdir(zooTargetDir) and "zootoolspro" in zooTargetDir:  # it exists already so delete for the upgrade
            ...
            try:
                ...
                message = "Could not delete folder {} \n" \
                           "This is likely locked due to a cloud service such as One Drive.\n" \
                           "Please manually delete the folder on disk and try again.\n\n" \
