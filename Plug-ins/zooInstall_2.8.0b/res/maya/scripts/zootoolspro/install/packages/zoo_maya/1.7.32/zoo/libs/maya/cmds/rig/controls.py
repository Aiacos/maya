...
def addTrackAttrs(transformName, scale=(1.0, 1.0, 1.0), color=None, rotate=(0.0, 0.0, 0.0),
    ...
    cmds.setAttr(".".join([transformName, controls.ZOO_TRANSLATE_DEFAULT_ATTR]), translate[0], translate[1], translate[2])
...
def deleteTrackAttrs(controlTransform):
    ...
    """
    attrList = controls.ZOO_CONTROLTRACK_VECTOR_LIST + controls.ZOO_CONTROLTRACK_RGB_LIST + controls.ZOO_CONTROLTRACK_STRING_LIST
...
def setTrackerScaleOffset(controlCurve, scaleXYZ):
    ...
    cmds.setAttr(".".join([controlCurve, controls.ZOO_SCALE_TRACK_ATTR]), trackerScale[0], trackerScale[1], trackerScale[2])
...
def duplicateControl(controlName, useSelectedName=False, copyScaleTracker=True, deleteNodeShapes=False):
    ...
    if cmds.nodeType(duplicatedCtrl) == "joint":  # is a joint so transfer shape to a transform node
        ...
        if copyScaleTracker:  # add the scale tracker attributes
            ...
            cmds.setAttr(".".join([duplicatedCtrl, controls.ZOO_SCALE_DEFAULT_ATTR]), zooScaleDefault[0], zooScaleDefault[1],
                         zooScaleDefault[2])
...
def reconnectAllBreakCtrls(message=True):
    ...
    networkNodeList = [s for s in allNetworkNodes if controls.ZOO_TEMPBREAK_NETWORK in s]  # returns all with break name matches
