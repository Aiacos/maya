...
class GuideDefinition(ControlDefinition):
    ...
    def update(self, other=None, **kwargs):
        ...
        currentChildren = {i["id"]: i for i in children}
        for requestedChild in data.get("children", []):
            existingChild = currentChildren.get(requestedChild["id"])
            if existingChild is None:
                children.append(GuideDefinition.deserialize(requestedChild))
            else:
                existingChild.update(requestedChild)
        if currentAttrs:
        ...
        self["children"] = children
        # super(GuideDefinition, self).update(data)
