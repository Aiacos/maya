...
class Rig(object):
    ...
    def rename(self, name):
        ...
        sets["ctrls"].rename(namingObject.resolve("selectionSet", {"rigName": name, "selectionSet": "ctrls",
                                                                   "type": "objectSet"}))
        sets["deform"].rename(namingObject.resolve("selectionSet", {"rigName": name, "selectionSet": "deform",
                                                                    "type": "objectSet"}))
        sets["root"].rename(namingObject.resolve("rootSelectionSet", {"rigName": name, "selectionSet": "rig",
                                                                      "type": "objectSet"}))
    ...
    def buildGuides(self, components=None):
        ...
        """
        self.configuration.updateConfigurationFromRig(self)
        childParentRelationship = {comp: comp.parent() for comp in self.iterComponents()}
    ...
    def _handleControlDisplayLayer(self, components):
        ...
        layer.playback = True
        for comp in components:
            for control in comp.rigLayer().iterControls():
