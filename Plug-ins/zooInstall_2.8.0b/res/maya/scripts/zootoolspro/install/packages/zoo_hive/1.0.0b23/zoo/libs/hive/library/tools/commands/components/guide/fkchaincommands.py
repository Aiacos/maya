...
class HiveSetFkParentGuides(command.ZooCommandMaya):
    ...
    isUndoable = True
    uiData = {"tooltip": __doc__,
              "label": "Set Fk parent guide ",
              }
    ...
    def doIt(self, parentGuide=None, childGuides=None):
        ...
        for index, [guide, currentGuideParent, _, _] in enumerate(self._guideMapping):
            ...
            annotation.delete(mod=modifier, apply=False)
            newAnnotation = guideLayers[0].createAnnotation(name, guide, parentGuide, guide)
    ...
    def undoIt(self):
        ...
        for guide, currentGuideParent, annotation, layer in self._guideMapping:
            ...
            if annotation:
                name = annotation.name()
                annotation.delete(mod=modifier, apply=False)
                layer.createAnnotation(name, guide, currentGuideParent, guide)
        modifier.doIt()
