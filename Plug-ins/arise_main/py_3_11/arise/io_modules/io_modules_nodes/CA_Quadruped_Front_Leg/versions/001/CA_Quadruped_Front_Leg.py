"""Cartoon_A_Quadruped_Front module creates a cartoon_complex quadruped front leg module. """
...
from arise.utils.modules_utils import (
    create_grps, movable_pivot_setup, create_bezier_ctrls, SECONDARY_COLOR, secondary_ctrls_setup, update_ctrls,
...
from arise.utils.subcomponents.ik_chain_three_joints import IkChainThreeJoints, TOGGLE_PV_ATTR
...
RIG_TYPE = "Quadruped"  # Biped, Car, Quadruped, All, ... (for filtering modules in lists).
TAGS = ["cartoon", "complex", "advance", "ribbon", "bendy bones", "ik", "fk", "leg", "quadruped"]
TOOL_TIP = "Cartoon quadruped front leg with IK/FK, ribbons, twist, soft IK, stretch, pin ctrl and more."
...
class CA_Quadruped_Front_Leg(node_data.NodeData):
    """Create a cartoon_complex quadruped front leg module. """
    sort_priority = 100
    ...
    def guides_creation(self):
        ...
        self.base_guide = self.add_guide(name="base", translation=[8.3, 125.1, 34.5], rotation=[0, 0, 180])
        self.base_guide.size = self.base_guide.size * 1.8

        self.root_guide = self.add_aim_guide(name="root", translation=[13.9, 97.4, 57.9], parent=self.base_guide)
        self.mid_guide = self.add_aim_guide(name="mid", translation=[13.5, 81.5, 43.1], parent=self.base_guide)
        self.mid_guide.visual_parent = self.root_guide

        self.tip_guide = self.add_guide(
            name="tip",
            translation=[12.5, 42.9, 45.5],
            rotation=[0, 0, 180],
        ...
        self.mid_guide.aim_rotation_offset = [0, 180, 0]

        self.root_guide.aim_side_pin = [self.tip_guide, self.ik_pv_guide]
        self.root_guide.aim_rotation_offset = [0, 180, 0]

        # foot guides.
        self.toes_guide = self.add_guide(
            name="toes_root",
            translation=[12.5, 15.4, 45.7],
            rotation=[0, 0, 180],
    ...
    def joints_creation(self):
        ...
        self.root_jnt = self.add_joint(name="root", skinning_jnt=False, radius=0.5)
        ...
        self.ik_toes_jnt = self.add_joint(name="ik_toes", skinning_jnt=False, radius=0.7)
        ...
        self.fk_toes_jnt = self.add_joint(name="fk_toes", skinning_jnt=False, radius=0.75)
    ...
    def ctrls_creation(self):
        ...
        self.base_ctrl = self.add_ctrl(
            name="base", shape="pin_sphere", up_orient="-X", size=(1.5 * scale_mult),
        )
        ...
        ## IK ctrls.
        self.ik_tip_ctrl = self.add_ctrl(name="ik_tip", shape="box", up_orient="+Y", size=(1.1 * scale_mult))
        ...
        self.fk_toes_ctrl = self.add_ctrl(name="fk_toes", shape="square", size=(1.5 * scale_mult))

        attrs = ["scaleY", "scaleZ"]
        if self.fk_translate_ctrls_attr.value is False:
            attrs = ["translateX", "translateY", "translateZ", "scaleY", "scaleZ"]

        for fk_ctrl in self.fk_three_ctrls + [self.fk_toes_ctrl]:
            for attr in attrs:
        ...
        if self.secondary_ctrl_attr.value:
            ...
            self.fk_toes_secondary = self.add_ctrl(name="fk_toes_secondary", size=(1.4 * scale_mult))

            for fk_secondary_ctrl in self.fk_secondary_ctrls + [self.fk_toes_secondary]:
                fk_secondary_ctrl.color = SECONDARY_COLOR

                for attr in attrs + ["scaleX"]:
        ...
        for ctrl in [self.ik_heel_ctrl, self.ik_toes_ball_ctrl, self.ik_toes_tip_ctrl]:
            for attr in ["scaleX", "scaleY", "scaleZ"]:
                ctrl.add_locked_hidden_attr(attr)

        for attr in ["translateX", "translateY", "translateZ"]:
            self.ik_toes_ball_ctrl.add_locked_hidden_attr(attr)

        self.ik_toes_ctrl = self.add_ctrl(name="ik_toes", shape="cylinder")
        self.ik_toes_ctrl.size = [(1.3 * scale_mult), (0.3 * scale_mult), (1.3 * scale_mult)]

        for attr in ["translateX", "translateY", "translateZ", "scaleY", "scaleZ"]:
            self.ik_toes_ctrl.add_locked_hidden_attr(attr)
    ...
    def maya_attrs_creation(self):
        ...
        self.ik_toes_ball_ctrl.add_maya_attr(name="meta_aim", attr_type="float", default_value=1, min=0, max=1)
        self.master_ctrl.add_maya_attr(name="foot_roll_angle", attr_type="float", default_value=30, min=0, max=180)
        self.master_ctrl.add_maya_attr(name="meta_tilt_mult", attr_type="float", default_value=0.5, min=0, max=1)
    ...
    def rig_creation(self):
        ...
        ik_chain_obj.tip_ctrl.add_spacer_attr()
        movable_pivot_setup(ik_chain_obj.tip_ctrl, scale_pivot=False, attr=None)
        ...
        base_vis_attr = ik_fk_switch_obj.create_attr_on_switch("base_ctrl", at="bool", k=True, dv=1)
        ...
        ik_toes_jnt = self.ik_toes_jnt.pointer
        fk_toes_jnt = self.fk_toes_jnt.pointer
        toes_jnt.parent_relative(ik_fk_switch_obj.tip_jnt)
        ik_toes_jnt.parent_relative(self.ik_tip_jnt.pointer)
        fk_toes_jnt.parent_relative(self.fk_tip_jnt.pointer)

        for jnt in [toes_jnt, ik_toes_jnt, fk_toes_jnt]:
            jnt.set_matrix(self.toes_guide.world_transformations["matrix"])
        ...
        ## move ik_tip_ctrl to toes_root_jnt position.
        ...
        toes_pos = toes_jnt.get_translation()

        if self.clean_transformations_attr.value:
            ik_chain_obj.tip_ctrl.offset_grp.set_translation(toes_pos)

        else:
            orient = self.world_orientation_attr.display_value
            ctrl_pos = ik_chain_obj.tip_ctrl.get_translation()
            distance = math_utils.distance_between(ctrl_pos, toes_pos)

            if "-" in orient:
                distance = distance * -1.0

            mult = 1.0
            if not self.mirror_behaviour_attr.value and self.is_mirrored:
                mult = -1.0

            trans_value = [
                distance * mult if "X" in orient else 0,
                distance if "Y" in orient else 0,
                distance if "Z" in orient else 0,
            ]
            mc.xform(ik_chain_obj.tip_ctrl.offset_grp, relative=True, os=True, translation=trans_value)

            ik_chain_obj.tip_ctrl.set_translation(toes_pos)
        ...
        ik_toes_tip_ctrl.offset_grp.parent_relative(tilt_inner_grp)
        ik_toes_tip_ctrl.offset_grp.set_matrix(toes_tip_jnt.get_matrix())

        ik_heel_ctrl = self.ik_heel_ctrl.pointer
        ik_heel_ctrl.offset_grp.parent_relative(ik_toes_tip_ctrl)
        ik_heel_ctrl.offset_grp.set_matrix(heel_pos_grp.get_matrix())

        ik_toes_ball_ctrl = self.ik_toes_ball_ctrl.pointer
        ik_toes_ball_ctrl.offset_grp.parent_relative(ik_heel_ctrl)
        ik_toes_ball_ctrl.offset_grp.set_matrix(ik_toes_jnt.get_matrix())

        ik_toes_ctrl = self.ik_toes_ctrl.pointer
        ik_toes_ctrl.offset_grp.parent_relative(ik_heel_ctrl)
        ik_toes_ctrl.offset_grp.set_matrix(ik_toes_jnt.get_matrix())

        ik_toes_jnt.orient_constraint_to(ik_toes_ctrl, mo=False)
        ik_toes_ctrl.offset_grp.point_constraint_to(ik_toes_jnt, mo=False)
        ik_toes_ctrl.scale_attrs_connect()
        ik_toes_jnt.connect_attr("scaleX", ik_toes_ctrl.attr("scaleX"))
        ik_toes_jnt.connect_attr("scaleY", ik_toes_ctrl.attr("scaleX"))
        ik_toes_jnt.connect_attr("scaleZ", ik_toes_ctrl.attr("scaleX"))

        loc_grp.parent(ik_toes_ball_ctrl)

        fk_toes_ctrl = self.fk_toes_ctrl.pointer
        fk_toes_ctrl.offset_grp.parent_relative(fk_chain_obj.tip_ctrl.btm_ctrl)
        fk_toes_ctrl.offset_grp.set_matrix(fk_toes_jnt.get_matrix())

        fk_toes_jnt.parent_constraint_to(fk_toes_ctrl.btm_ctrl, mo=False)
        fk_toes_jnt.scale_constraint_to(fk_toes_ctrl.btm_ctrl, mo=False)
        fk_toes_ctrl.scale_attrs_connect()

        switch_attr = ik_fk_switch_obj.switch_attr.rsplit(".", 1)[-1]
        switch_ctrl = ik_fk_switch_obj.switch_ctrl if ik_fk_switch_obj.switch_ctrl else ik_chain_obj.tip_ctrl
        switch_attr = "{0}.{1}".format(switch_ctrl, switch_attr)

        parent_constraint = toes_jnt.parent_constraint_to([ik_toes_jnt, fk_toes_jnt], maintainOffset=False)
        ik_attr, fk_attr = mc.parentConstraint(parent_constraint, q=True, weightAliasList=True)
        mc.connectAttr(ik_fk_switch_obj.switch_attr_reverse, "{0}.{1}".format(parent_constraint, ik_attr))
        mc.connectAttr(switch_attr, "{0}.{1}".format(parent_constraint, fk_attr))

        blend_node = mc.createNode("blendColors", n="{0}_toes_scale_ik_fk_switch_blend".format(self.name))
        mc.connectAttr("{0}.scaleX".format(fk_toes_jnt), "{0}.color1R".format(blend_node))
        mc.connectAttr("{0}.scaleY".format(fk_toes_jnt), "{0}.color1G".format(blend_node))
        mc.connectAttr("{0}.scaleZ".format(fk_toes_jnt), "{0}.color1B".format(blend_node))

        mc.connectAttr("{0}.scaleX".format(ik_toes_jnt), "{0}.color2R".format(blend_node))
        mc.connectAttr("{0}.scaleY".format(ik_toes_jnt), "{0}.color2G".format(blend_node))
        mc.connectAttr("{0}.scaleZ".format(ik_toes_jnt), "{0}.color2B".format(blend_node))

        mc.connectAttr(switch_attr, "{0}.blender".format(blend_node))

        mc.connectAttr("{0}.outputR".format(blend_node), "{0}.scaleX".format(toes_jnt))
        mc.connectAttr("{0}.outputG".format(blend_node), "{0}.scaleY".format(toes_jnt))
        mc.connectAttr("{0}.outputB".format(blend_node), "{0}.scaleZ".format(toes_jnt))

        if self.expose_rotation_order_attr.value:
            for ctrl in [ik_toes_ctrl, fk_toes_ctrl, fk_toes_ctrl.btm_ctrl]:
                mc.setAttr("{0}.rotateOrder".format(ctrl), channelBox=True)

        # meta_aim setup.
        meta_offset_grp = ik_toes_ball_ctrl.add_group_above("{0}_meta_aim_offset_grp".format(self.name))
        meta_grp = ik_toes_ball_ctrl.add_group_above("{0}_meta_aim_grp".format(self.name))
        offset_b_grp = ik_toes_ball_ctrl.add_group_above("{0}_offset_b_grp".format(ik_toes_ball_ctrl.short_name))
        offset_b_grp.parent(self.module_grp)

        mc.delete(
            meta_offset_grp.aim_constraint_to(
                ik_chain_obj.root_jnt,
                aimVector=(0, -1, 0),
                upVector=(0, 0, -1),
                worldUpType="object",
                worldUpObject=ik_chain_obj.pv_ctrl,
                maintainOffset=False,
            )
        )

        aim_constraint = meta_grp.aim_constraint_to(
            ik_chain_obj.root_jnt,
            aimVector=(0, -1, 0),
            upVector=(0, 0, -1),
            worldUpType="object",
        ...
        offset_b_grp.parent(meta_grp)

        ik_toes_ball_ctrl.add_spacer_attr()
        meta_aim_attr = ik_toes_ball_ctrl.add_attr("meta_aim", min=0, max=1, dv=1, keyable=True)

        if self.add_toggle_pv_attr.value:
            active_node = mc.createNode(MULT_DL, n="{0}_meta_aim_active_mult".format(self.name))
            mc.connectAttr(meta_aim_attr, "{0}.{1}".format(active_node, MULT_DL_INPUT1))
            pv_enable_attr = "{0}.{1}".format(ik_chain_obj.tip_ctrl, TOGGLE_PV_ATTR)
            mc.connectAttr(pv_enable_attr, "{0}.{1}".format(active_node, MULT_DL_INPUT2))
            output_attr = "{0}.output".format(active_node)
        else:
            output_attr = meta_aim_attr

        mult_node = mc.createNode("multiplyDivide", n="{0}_meta_aim_mult".format(self.name))
        mc.connectAttr("{0}.constraintRotateX".format(aim_constraint), "{0}.input1X".format(mult_node))
        mc.connectAttr("{0}.constraintRotateY".format(aim_constraint), "{0}.input1Y".format(mult_node))
        mc.connectAttr("{0}.constraintRotateZ".format(aim_constraint), "{0}.input1Z".format(mult_node))

        mc.connectAttr(output_attr, "{0}.input2X".format(mult_node))
        mc.connectAttr(output_attr, "{0}.input2Y".format(mult_node))
        mc.connectAttr(output_attr, "{0}.input2Z".format(mult_node))

        mc.connectAttr("{0}.outputX".format(mult_node), "{0}.rotateX".format(meta_grp), f=True)
        mc.connectAttr("{0}.outputY".format(mult_node), "{0}.rotateY".format(meta_grp), f=True)
        mc.connectAttr("{0}.outputZ".format(mult_node), "{0}.rotateZ".format(meta_grp), f=True)
        ...
        meta_tilt_attr = master_ctrl.add_attr("meta_tilt_mult", dv=0.5, min=0, max=1, keyable=True)
        ...
        mc.connectAttr(master_ctrl.attr("rotateY"), "{0}.inputB".format(clamp))
        mc.setAttr("{0}.maxR".format(clamp), 360)
        mc.setAttr("{0}.minG".format(clamp), -360)
        mc.setAttr("{0}.minB".format(clamp), -360)
        mc.setAttr("{0}.maxB".format(clamp), 360)
        ...
        meta_tilt_mult = mc.createNode(MULT_DL, n="{0}_meta_tilt_mult".format(self.name))
        mc.connectAttr(meta_tilt_attr, "{0}.{1}".format(meta_tilt_mult, MULT_DL_INPUT1))
        mc.connectAttr("{0}.outputB".format(clamp), "{0}.{1}".format(meta_tilt_mult, MULT_DL_INPUT2))
        mc.connectAttr("{0}.output".format(meta_tilt_mult), toes_driven_grp.attr("rotateZ"))
