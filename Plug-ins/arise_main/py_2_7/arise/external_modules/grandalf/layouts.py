...
from arise.external_modules.grandalf.utils import *
from arise.external_modules.grandalf.utils import geometry
...
class SugiyamaLayout(object):
    ...
    *dot* in graphviz. This layout is quite efficient but heavily relies
    ...
    def __init__(self, g):
        ...
        self.dw, self.dh = geometry.median_wh([v.view for v in self.g.V()])
        self.initdone = False
    ...
    def init_all(self, roots=None, inverted_edges=None, optimize=False):
        ...
        if roots == None:
            roots = [v for v in self.g.V() if len(v.e_in()) == 0]
        if inverted_edges == None:
            L = self.g.get_scs_with_feedback(roots)
            inverted_edges = [x for x in self.g.E() if x.feedback]
...
class DigcoLayout(object):
    ...
    def optimal_arrangement(self):
        b = self.balance()
        y = DigcoLayout.geometry.rand_ortho1(self.g.order())
    ...
    def balance(self):
        b = DigcoLayout.geometry.array([0.0]*self.g.order(), dtype=float)
        for e in self.g.E():
            s = e.v[0]
            d = e.v[1]
    ...
    def _conjugate_gradient_L(self, y, b):
        ...
        p = DigcoLayout.geometry.array(r, copy=True)
        rr = sum(r*r)
    ...
    def _xyinit(self, y=None):
        if y is None:
            y = DigcoLayout.geometry.rand_ortho1(self.g.order())
        x = DigcoLayout.geometry.rand_ortho1(self.g.order())
        # translate and normalize:
        x = x-x[0]
        y = y-y[0]
        sfactor = 1.0/max(list(map(abs, y))+list(map(abs, x)))
        return DigcoLayout.geometry.matrix(list(zip(x*sfactor, y*sfactor)))
    ...
    def __Lii_(self):
        ...
        return DigcoLayout.geometry.array(Lii, dtype=float)
    ...
    def _cg_Lw(self, Lw, z, b):
        def scal(U, V): return float(U.transpose()*V)
        r = b - Lw*z
        print(r)
        p = copy.copy(r)
        rr = scal(r, r)
    ...
    def __Dij_(self):
        ...
        return DigcoLayout.geometry.matrix(Dji, dtype=float)
    ...
    def __Lij_Z_Z(self, Z):
        ...
        liz = DigcoLayout.geometry.matrix([0.0]*n)  # liz is a row of L^Z (size n)
        # compute lzz = L^Z.Z while assembling L^Z by row (liz):
        for i in xrange(n):
            iterk_except_i = (k for k in xrange(n) if k != i)
            for k in iterk_except_i:
                v = Z[i]-Z[k]
                liz[0, k] = 1.0/(self.Dij[i, k]*DigcoLayout.geometry.sqrt(v*v.transpose()))
