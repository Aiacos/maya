...
from adn.utils.constants import DeformerTypes, ConstraintTypesNiceNames
...
def get_anchors_and_deformer(selection):
    ...
    node_history = cmds.listHistory(deformer_object)
...
def add_geometry_target(deformer=None, attachments=None):
    ...
    # Remove attachments that are being deformed by an AdnMuscle or AdnRibbonMuscle
    for attachment in attachments:
        deformers_affecting_attachment = cmds.findDeformers(attachment) or []
        for deformer in deformers_affecting_attachment:
            if cmds.nodeType(deformer) in [DeformerTypes.RIBBON, DeformerTypes.MUSCLE]:
                unsuccessful_attached.append(attachment)
                short_name = cmds.ls(attachment, shortNames=True)[0]
                print("{0} Geometry {1} is deformed by {2}. Currently, using "
                      "simulated muscles as targets is not supported."
                      "".format(UiConstants.LOG_PREFIX, short_name, deformer))
...
def remove_geometry_target(deformer=None, attachments=None):
    ...
    for i in range(0, len(connected_indices)):
        ...
        if matrix_found and mesh_found:
            index_to_remove.append(i)
            # Keep track of node pairs forming an attachment to avoid removing
            # attachments which contain just 1 of them. Eg. Selecting two nodes forming
            # an attachment but removing 3 attachments containing part of that node
            discarded_nodes.append(connected_attachments[i][0])
            discarded_nodes.append(connected_attachments[i][1])
...
def add_targets():
    ...
    if not deformer or not attachments:
        msg_box(main_window(), "error",
                "No valid attachments and deformer provided. Please retry adding the attachments."
    ...
    if unsuccessful_attached and not successful_attached: # All attachments are skipped
        unsuccessful_attached = ["\n    \"{0}\"".format(s) for s in unsuccessful_attached]
        attachment_names = "".join(unsuccessful_attached)
        msg_box(main_window(), "warning",
                ("No new attachments connected to \"{0}\". "
                 "\nAll selected nodes could not be connected or are already "
                 "connected to the deformer: {1}\nPlease, check the console "
                 "for more information and retry."
                 "".format(deformer_short_name, attachment_names)))
        return
    elif not successful_attached: # No connected attachments (no skipping)
        msg_box(main_window(), "error",
                ("No new attachments connected to \"{0}\". Please, retry."
                 "".format(deformer_short_name)))
        return
    elif unsuccessful_attached: # Some attachments skipped, some added
        successful_attached = ["\n    \"{0}\"".format(s) for s in successful_attached]
        successful_attachment_names = "".join(successful_attached)
        unsuccessful_attached = ["\n    \"{0}\"".format(s) for s in unsuccessful_attached]
        skip_attachment_names = "".join(unsuccessful_attached)
        msg_box(main_window(), "warning",
                ("Attachments connected successfully to \"{0}\":"
                 "{1}\nThe following attachments are already connected to the "
                 "deformer or could not be successfully added: {2}\nPlease, check "
                 "the console for more information and retry."
                 "".format(deformer_short_name, successful_attachment_names, skip_attachment_names)))
    else: # All added, no skipped
        successful_attached = ["\n    \"{0}\"".format(s) for s in successful_attached]
        attachment_names = "".join(successful_attached)
        msg_box(main_window(), "info",
                ("Attachments connected successfully to \"{0}\":"
                 "{1}").format(deformer_short_name, attachment_names))
...
def remove_targets():
    ...
    if not deformer:
        msg_box(main_window(), "error",
                "Could not find a valid deformer from the provided selection."
    ...
    if not attachments:
        ...
        if painting_deformer and (weights_type == ConstraintTypesNiceNames.ATTACHMENT or
            ...
            msg_box(main_window(), "error", painting_deformer_error_message)
            return
    ...
    if not unsuccessful_dettached and not successful_dettached:
        ...
        if not unsuccessful_dettached:
            msg_box(main_window(), "error",
                "Selected deformer has no attachments to remove.")
            return

    if unsuccessful_dettached and successful_dettached:
        unsuccessful_dettached = ["\n    \"{0}\"".format(s) for s in unsuccessful_dettached]
        successful_dettached = ["\n    \"{0}\"".format(s) for s in successful_dettached]
        msg_box(main_window(), "warning",
                ("The disconnect attachment action for \"{0}\" did not have any effect for:"
                "{1}\n The following attachments disconnected successfully:"
                "{2}" ).format(deformer_short_name, "".join(unsuccessful_dettached), "".join(successful_dettached)))
        ...
        msg_box(main_window(), "info",
                ("Attachments disconnected successfully from \"{0}\":"
                "{1}").format(deformer_short_name, attachment_names))
...
def add_slide_segment_constraint():
    ...
    if not current_selection:
        msg_box(main_window(), "error",
                "Selection is empty. Please, make sure that you select an "
    ...
    if not deformer or not transforms:
        msg_box(main_window(), "error",
                "No valid transform nodes and deformer provided."
                "\nPlease, select the elements in the following order "
                "and try again:"
    ...
    if len(transforms) < 2:
        msg_box(main_window(), "error",
                "Please select at least two transform nodes to define a segment. "
                "Then, the mesh with AdonisFX deformer applied to.")
        return
    
    if not scene_parent_check(transforms):
        msg_box(main_window(), "error",
                "The current provided selection does not satisfy this condition for slide on segment constraints."
    ...
    if duplicated_segments and not successful_attached: # All anchors are skipped 
        duplicated_segments = ["\n    \"{0} - {1}\"".format(cmds.ls(s[0], shortNames=True)[0],
                                                            cmds.ls(s[1], shortNames=True)[0]) for s in duplicated_segments]
        segments_names = "".join(duplicated_segments)
        msg_box(main_window(), "warning",
                ("No new segments connected to \"{0}\". "
                 "\nAll segments are already connected to the deformer:"
                 "{1}".format(deformer_short_name, segments_names)))
        return
    elif not successful_attached: # No connected segments (no skipping) 
        msg_box(main_window(), "error",
                ("No new segments connected to \"{0}\". Please, retry."
                 "".format(deformer_short_name)))
        return
        ...
        msg_box(main_window(), "warning",
                ("Segments connected successfully to \"{0}\":"
                 "{1}\nThe following segments are already connected to the deformer:"
                 "{2}".format(deformer_short_name, successful_anchor_names, skip_segments_names)))
    else: # All added, no skipped
        successful_attached = ["\n    \"{0} - {1}\"".format(cmds.ls(s[0], shortNames=True)[0],
                                                            cmds.ls(s[1], shortNames=True)[0]) for s in successful_attached]
...
def remove_slide_segment_constraint():
    ...
    if deformer_being_painted(deformer_short_name):
        ...
        if weights_type == ConstraintTypesNiceNames.SLIDE_SEGMENT:
            msg_box(main_window(), "error",
                    "Removing slide on segments constraints is not allowed while painting. "
                    "Please exit from the paint context and try again.")
            return
    ...
    if not connected_indices:
        msg_box(main_window(), "error",
                "The selected deformer has no segments to remove.")
        return
    ...
    if unsuccessful_dettached:
        unsuccessful_dettached = ["\n    \"{0}\"".format(cmds.ls(s, shortNames=True)[0]) for s in unsuccessful_dettached]
        attachment_names = "".join(unsuccessful_dettached)
        msg_box(main_window(), "error",
                ("The disconnect segment action for \"{0}\" did not have any effect for:" 
                 "{1}").format(deformer_short_name, attachment_names))
    if successful_dettached:
        ...
        attachment_names = "".join(successful_dettached)
        msg_box(main_window(), "info",
                ("Segments disconnected successfully from \"{0}\":" 
                 "{1}").format(deformer_short_name, attachment_names))
