...
from adn.utils.constants import DebugDataDescriptor 
...
def apply_muscle(custom_name=None, start_time=None, stiffness=1e5, iterations=10, gravity=0.0, damping=0.75):
    ...
    for applied_deformer in applied_deformers:
        if cmds.nodeType(applied_deformer) == DeformerTypes.MUSCLE:
            msg_box(main_window(), "warning",
                    "The selected mesh ({0}) has already an AdnMuscle deformer node "
    ...
    if unsuccessful_attachments:
        ...
        msg_box(main_window(), "warning",
                ...
                "".format(muscle_deformer_short_name, attachment_names))
    else:
        msg_box(main_window(), "info",
                "\"{0}\" deformer has been created successfully."
                "".format(muscle_deformer_short_name))
