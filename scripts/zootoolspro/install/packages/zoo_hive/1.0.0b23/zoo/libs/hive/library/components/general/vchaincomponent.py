...
class VChainComponent(api.Component):
    ...
    def alignGuides(self):
        ...
        if endGuide.autoAlign.asBool():
            if self._resetEndGuideAlignment:
                ...
                midRotation = midGuide.rotation(zapi.kWorldSpace)
        ...
        if not upVecGuide.autoAlign.value():
            return True

        with zapi.lockStateAttrContext(upVecGuide,
                                       zapi.localTransformAttrs + ["translate", "rotate", "scale"], False):
            # update the upVec
            try:
                newPos = skeletonutils.poleVectorPosition(*positions)
            except ValueError:
                newPos = upVecGuide.translation()
            upVecGuide.setRotation(zapi.Quaternion())
            if newPos != zapi.Vector():
                upVecGuide.setTranslation(newPos, space=zapi.kWorldSpace)
    ...
    def setupInputs(self):
        ...
        upVecInMatrix = guideLayerDef.guide("upVec").transformationMatrix(scale=False)
    ...
    def setupRig(self, parentNode):
        ...
        ctrl = rigLayer.createControl(name=upVecName,
                                      id=upvecGuide.id,
        ...
        rigLayer.createSrtBuffer(upvecGuide.id, "_".join([upVecName, "srt"]))
        self._ikCtrlsTuple["upvec"] = ctrl
