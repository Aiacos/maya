...
class ExecutorBase(object):
    def __init__(self, interface=None, registerEnv="ZOO_COMMAND_LIB"):
        interface = interface or [command.ZooCommand]
    ...
    def execute(self, name, *args, **kwargs):
        ...
        try:
            ...
            if command.requiresWarning():
                output.displayWarning(command.warningMessage())
                return
    ...
    def _callDoIt(self, cmd):
        try:
            ...
            cmd._returnStatus = command.RETURN_STATE_SUCCESS
        except Exception:
            traceback.print_exception(*sys.exc_info())
            cmd._returnResult = None
            cmd.returnStatus = command.RETURN_STATE_ERROR
