...
from zoo.libs.maya.cmds.animation import generalanimation, grapheditorfcurve, keyframes
...
def lightMarkingMenuPress(alt=False, shift=False, ctrl=False):
    ...
    zooMenu.MarkingMenu.buildFromLayout(
        "markingMenu.lights",
        "lightsMarkingMenu",
...
def selectionSetMarkingMenuPress(alt=False, shift=False, ctrl=False):
    ...
    zooMenu.MarkingMenu.buildDynamicMenu(
...
def open_zooShelfFloatingWindow():
    from zoo.apps.toolpalette import run
    run.currentInstance().executePluginById("zoo.shelf.zooFloatingShelf")
