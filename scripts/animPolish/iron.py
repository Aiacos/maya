...
def run_sel (hideData = 1, smooth = 3):
	...
	if sel:
		...
		cmds.select (geos)
		if worked >= 2:
			sys.stdout.write ("Created 'Smooth' on multiple objects.")
	else:
...
def run (geo, dfrmverts = [], floodverts = [], cb = '', hideData = 1, smooth = 3, its = 3):
	...
	dup = cmds.duplicate (geo, n = '{}_iron_{}_LIVE'.format (geo,num), ic = 1, rr = 1)[0]
	...
	if par:
		cmds.parent (dup, w = 1)
	'''
	bs = cmds.blendShape (dup, geo, n = '{}_iron_{}_BS'.format (geo.split ('|')[-1],num), o = 'world', w = [0,1])[0]

	# Add attribute control
	ln = 'iron_{}'.format (num)
	cmds.addAttr (geo, at = 'float', dv = 1, k = 1, ln = ln, min = 0, max = 1)

	# Create polyAverageVertex
	avs = []
	for i in range (its):
		av = cmds.polyAverageVertex (dfrmverts, i = 15)[0]
		avs.append (av)
	remap = cmds.createNode ('remapValue', n = '{}__{}__RMP'.format (geo,ln))
	cmds.setAttr ('{}.inputMin'.format (remap), 0)
	cmds.setAttr ('{}.inputMax'.format (remap), 1)
	cmds.setAttr ('{}.outputMin'.format (remap), 0)
	cmds.setAttr ('{}.outputMax'.format (remap), 15)
	cmds.connectAttr ('{}.iron_{}'.format (geo,num), '{}.inputValue'.format (remap))
	for av in avs:
		cmds.connectAttr ('{}.outValue'.format (remap), '{}.iterations'.format (av))
	if hideData == 1:
		cmds.setAttr ('{}.ihi'.format (remap), 0)
		for av in avs:
			cmds.setAttr ('{}.ihi'.format (av), 0)
	...
	cmds.setAttr ('{}.v'.format (dup), 0)
	cmds.select (geo)
	sys.stdout.write ("Created 'Iron' on '{}'.".format (geo.split ('|')[-1]))
	worked = 1
